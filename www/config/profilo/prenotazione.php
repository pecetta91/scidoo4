...
if($gg>0){
	...
	$IDserv=0;
	$query="SELECT ID FROM prenextra WHERE IDpren='$IDpren' AND tipolim='5' LIMIT 1";
	$result=mysqli_query($link2,$query);
	if(mysqli_num_rows($result)>0){
		$row=mysqli_fetch_row($result);
		$IDserv=$row['0'];
	}else{
		$query="SELECT ID FROM prenextra WHERE IDpren='$IDpren' ORDER BY prezzo DESC LIMIT 1";
		$result=mysqli_query($link2,$query);
		$row=mysqli_fetch_row($result);
		$IDserv=$row['0'];
	}
	$foto='big'.getfoto($IDserv,4);
}
$foto='immagini/'.$foto;
...
<table style="width:95%; margin:auto; "><tr><td style="line-height:15px;font-size:19px;color:#2b4795; font-weight:400">'.$nomestr.'</td><td align="right" valign="top"><b style="font-size:17px;color:#2b4795; ">'.round($totale,1).' €</b></td></tr></table>
