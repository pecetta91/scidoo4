...
';
	...
	if(mysqli_num_rows($result)>0){
		...
		$sala='';
			...
			$txt='<li class="accordion-item" style="padding:0px;">
				  <a href="#" class="item-link item-content" onclick="navigation(25,'.$ID.',0,0)">
				   ...
				   <div class="accordion-item-content" style="padding:0px; font-size:13px; "><br>
						<div class="content-block"  class="details" style="padding:0px; text-align:center;">
							
						<div style="font-size:13px; width:95%; margin:auto; text-align:center;">'.stripslashes($descr).'</div>
					<br>
					<p class="buttons-row">';
					
					if($tipolim==2){	
					
						//if(($time-3600*4)>$timeora){
							$txt.='<a href="#" onclick="modificaorario('.$ID.',1,'.$time.',0)" class="button button-raised button-fill color-green" style="width:47%; margin:auto;">Orario</a>';
						//}
						
						
					}
						
					if((($tipolim!=4)||($tipolim!=5))&&(($time-3600*4)>$timeora)&&($pacchetto=='0')){
						$txt.='<a href="#" onclick="msgboxelimina('.$ID.',32,0,0,0)" class="button button-raised button-fill color-red" style="width:47%; margin:auto;">Elimina</a>';
					...
					<div style="font-size:10px; padding:5px; color:#666;text-align:left;">(*) Potr&agrave; modificare l&acute;orario del servizio fino a 4hr prima del suo inizio.
...
foreach ($serviziarr as $data =>$cont){
	...
	$testo.='
<div class="content-block-title" style="font-size:14px; color:#2b57af; font-weight:600;">'.dataita($time).' '.date('Y',$time).'</div>
			<div class="list-block media-list accordion-list" style="padding:0px;">
			  <ul>'.$cont.'
			  </ul></div>
	';
