...
$selectedf='';
$selectedm='';
$placeholdertel=' placeholder="Inserire Telefono" ';
$placeholdermail=' placeholder="Inserire Email" ';
...
if($IDriga==$ID)
{
	...
	$placeholdertel='';
	$placeholdermail='';
	$prefisso='style="pointer-events: none;"';
}
	...
	case 2:
		$query="SELECT nome,cognome,tel,mail,datanas,sesso,note,noteristo,cell,prefissotel,prefissocell,cittadinanza,luogonas,residenza,indirizzo,documento,numero,dataril,luogoril FROM schedine WHERE ID='$IDcliente' LIMIT 1";
			...
			if(mysqli_num_rows($result)>0){
				...
				if($datanas=='0000-00-00'){
					$datanas=date('Y-m-d',$time0);
				}
				...
				$cittadinanzaver='<span class="placeholderauto">Inserire Cittadinanza</span>';
				$luogonasver='<span class="placeholderauto">Inserire luogo di Nascita</span>';
				$residenzaver='<span class="placeholderauto">Inserire luogo di Residenza</span>';
				$documentover='<span class="placeholderauto"">Inserire Documento</span>';
				$luogorilver='<span class="placeholderauto">Inserire luogo Rilascio documento</span>';
				...
				$luogonas=$row['12'];
					...
					if(mysqli_num_rows($result2)>0){
						...
						$query3="SELECT cod,descrizione,provincia FROM alloggiati.stati WHERE cod='$luogonas' ORDER BY descrizione LIMIT 1";
						$result3=mysqli_query($link2,$query3);

						if(mysqli_num_rows($result3)>0){
								$row3=mysqli_fetch_row($result3);
								$luogonasver=ucwords(strtolower($row3['1']));
						}
					}
				...
				$residenza=$row['13'];
					...
					if(mysqli_num_rows($result2)>0){
						...
						$query3="SELECT cod,descrizione,provincia FROM alloggiati.stati WHERE cod='$residenza' ORDER BY descrizione LIMIT 1";
						$result3=mysqli_query($link2,$query3);

						if(mysqli_num_rows($result3)>0){
								$row3=mysqli_fetch_row($result3);
								$residenzaver=ucwords(strtolower($row3['1']));
						}
					}
				...
				if($dataril=='0000-00-00'){
					$dataril=date('Y-m-d',$time0);
				}
				...
				$luogoril=$row['18'];
					...
					if(mysqli_num_rows($result2)>0){
						...
						$query3="SELECT cod,descrizione,provincia FROM alloggiati.stati WHERE cod='$luogoril' ORDER BY descrizione LIMIT 1";
						$result3=mysqli_query($link2,$query3);

						if(mysqli_num_rows($result3)>0){
								$row3=mysqli_fetch_row($result3);
								$luogorilver=ucwords(strtolower($row3['1']));
						}
					}
			}
...
/*
					...
					$testo.='
						...
						<li><a href="#" class="item-link smart-select" data-open-in="picker">
							<select name="sesso"  onchange="modprofilo('.$IDcliente.',this,25,11)">
							  <option value="M" '.$selectedm.'>Maschio</option>
							  <option value="F" '.$selectedf.'>Femmina</option>
							</select>
							<div class="item-content">
							  <div class="item-inner">
						...
						$testo.='<div class="list-block accordion-list">
						<ul>
						
						  <li class="accordion-item"><a href="#"  class="item-content item-link">
							<div class="item-inner">
							  <div class="item-title label">Data di Nascita</div>
							  <div class="item-after"><input type="text" readonly   style="display:none;" onchange="modprofilo('.$IDcliente.','."'datanascita'".',24,0)" id="datanascita" value="'.$datanas.'">
							  <span id="datanascitatxt">'.convertidata3($datanas,'SI').'</span>
						...
						<li>
						  <div class="item-content">
							<div class="item-inner">
							  ...
							  <div class="item-input" >
								<input type="text" id="numerodoc" onchange="modprofilo('.$IDcliente.','."'numerodoc'".',27,0)" value="'.$numerodoc.'" placeholder="Inserire numero documento"  >
							  </div>
							</div>
						  </div>
						</li>
						 ...
						 <li class="accordion-item"><a href="#"  class="item-content item-link">
							<div class="item-inner">
							  <div class="item-title label">Data di Rilascio</div>
							  <div class="item-after"><input type="text" readonly   style="display:none;" onchange="modprofilo('.$IDcliente.','."'dataril'".',33,0)" id="dataril" value="'.$dataril.'">
							  <span id="datariltxt">'.convertidata3($dataril,'SI').'</span>
				...
				$testo.='<div class="list-block">
						...
						<li>
						  <div class="item-content">
							<div class="item-inner">
							  ...
							  <div>
								  <a href="#" class="item-link smart-select" data-open-in="picker" pickerHeight="400px" '.$prefisso.'>
									<select class="preftelpicker" id="prefisso" onchange="modprofilo('.$IDcliente.',this,38,11)">'.generaprefisso($prefissotel).' </select>
								 </a>
								</div>
							  <div class="item-input" >
								<input type="tel" id="tel"  onchange="modprofilo('.$IDcliente.','."'tel'".',35,0)" value="'.$tel.'" '.$modificabile.'  '.$placeholdertel.'>
							  </div>
							</div>
						  </div>
						</li>
						<li>
						  <div class="item-content">
							<div class="item-inner">
							  ...
							  <div class="item-input" >
								<input type="email" id="email"  onchange="modprofilo('.$IDcliente.','."'email'".',34,0)" value="'.$email.'" '.$modificabile.'  '.$placeholdermail.'>
							  </div>
							</div>
						  </div>
						</li>
								...
								<li>
									<div class="item-content h100">
										<div class="item-inner h100" >
										  <div class="item-input">
											<textarea  class="textareanew textareapren" placeholder="Note ristorante" id="noteristo"  onchange="modprofilo('.$IDcliente.','."'noteristo'".',37,0)" >'.$noteristo.'</textarea>
										  </div>
										</div>
									</div>
								</li>
						...
						<p class="privacyinfo">
							...
							Potranno essere utilizzati soltanto dalla struttura ricettiva per fini statistici e di sicurezza.
							</p>
							<br><br><br>
...
/*<form class="searchbar searchbar-init ricercanuovoservform">
						<div class="searchbar-input">
						  <input type="search" placeholder="Search"  class="ricercainput"><a href="#" class="searchbar-clear"></a>
