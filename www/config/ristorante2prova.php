...
if($not['0']>0){
	$not1='<div class="notific4">'.$not['0'].'</div>';
		...
		<table>
		<tr>
		<td><a '.$check1.' class="tab-link buttonristo" onclick="tavolisosp('.$time.','.$IDsottotip.',1)">Tavoli In Arrivo '.$not1.'</a></td>
		...
		';
					...
					foreach($sale as $IDsala =>$nomesala){
						...
						$testo.='<td>
						<a href="#IDsalaristo'.$IDsala.'"  onclick="IDtabac2='."'IDsalaristo".$IDsala."';".'" class="tab-link buttonristo   '.$active.' " style="overflow:visible;">'.$nomesala.'
						'.$badge.'
						</a> </td>';
