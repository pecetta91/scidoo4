...
switch($tipo){
	...
	<li>
      <a href="#"  class="item-link  smart-select" data-open-in="picker"  pickerHeight="400px" data-searchbar="false" >
	...
	<li>
      ...
      <a href="#" class="item-link  smart-select" data-open-in="picker"  pickerHeight="400px" data-searchbar="false">
          ...
          <div class="item-inner">
            ...
            <div class="item-after">'.$tempn.'</div>
				...
				if(mysqli_num_rows($result5)>0){	
							$testo.='<div class="content-block-title"  style="color:#2d4e99;"><b>Altre Informazioni</b></div>
									<div class="list-block">
										<ul>';
						while($row5=mysqli_fetch_row($result5)){
							
							$IDrestr=$row5['0'];
							$query3="SELECT ID,nome FROM infopren WHERE IDrest='$IDrestr' AND IDstr='$IDstruttura' AND IDpren='$id' LIMIT 1";
							$result3=mysqli_query($link2,$query3);
							if(mysqli_num_rows($result3)>0){
								$row3=mysqli_fetch_row($result3);
								$num=$row3['1'];
							...
							$testo.='<li>
								  <a href="#" class="item-link  smart-select" data-open-in="picker" pickerHeight="400px" data-back-on-select="true" data-searchbar="false">
									<select  id="restriz'.$row5['0'].'" onchange="modprenot('."'".$row5['0']."_".$id."'".',this,4,11)" alt="'.$row5['0'].'" class="selectdx inputrestr" >
							  '.generanum(0,5,$num).'</select>
									<div class="item-content">
									  <div class="item-inner">
		...
		$insda='';
		$query="SELECT p2.nome FROM prenotazionins as p,personale as p2 WHERE p.IDpren='$id' AND p.IDpers=p2.ID LIMIT 1";
		$result=mysqli_query($link2,$query);
		...
		$insdove='';
		$query="SELECT luogo FROM prenotazionidove WHERE IDpren='$id' LIMIT 1";
		$result=mysqli_query($link2,$query);
		if(mysqli_num_rows($result)>0){
			$row=mysqli_fetch_row($result);
			switch($row['0']){
		...
		$testo.='<div class="content-block-title"  style="color:#2d4e99;"><b>Altre Informazioni</b></div><div style="padding-left:15px; color:#333; font-size:13px; line-height:15px;">
		'.$insda.'</br><b>- Dove: </b>'.$insdove.'<br/>
		
		- <b>Inserita il: </b> '.dataita4($datapren).' '.date('H:i',$datapren).'</br></div><br/><br/>
	...
	*/
	//per tipo
	$query="SELECT p.ID,p.time,p2.datacar,COUNT(DISTINCT(p2.IDprenextra)),GROUP_CONCAT(DISTINCT(p.ID) SEPARATOR ','),p.tipolim,p.extra,p.modi,SUM(p2.prezzo),GROUP_CONCAT(DISTINCT(p2.IDinfop) SEPARATOR ','),SUM(p2.qta),p.sala,p.IDtipo,p.sottotip FROM prenextra as p,prenextra2 as p2 WHERE p2.pacchetto<='0' AND p2.IDpren IN ($IDprenc) AND p.ID=p2.IDprenextra AND p2.paga>'0' AND p.IDtipo!='19' $qadd2 GROUP BY p.extra ORDER BY p.sottotip";
