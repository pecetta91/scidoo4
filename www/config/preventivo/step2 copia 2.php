...
if($gg==0){
	...
	$qadd='';
	 ...
	 if($_SESSION['appnew']!=0){
		...
		if(!in_array($_SESSION['appnew'],$arrg)){
			$explode=1;
			$qadd=" AND a.ID='".$_SESSION['appnew']."'";
		}else{
	...
	if($explode==0){
		...
		$testo.='<br><div style="width:100%; text-align:center; margin-bottom:-20px;">Alloggio: <b>'.$row2['0'].'</b><br>
	...
	$query="SELECT a.nome,a.categoria,a.ID,c.colore FROM appartamenti as a,categorie as c WHERE a.ID NOT IN($groupid)  AND a.attivo>='0' AND a.IDstruttura='$IDstruttura' AND a.categoria=c.ID $qadd ORDER BY a.categoria";
	$result=mysqli_query($link2,$query);
	if(mysqli_num_rows($result)>0){
		while($row=mysqli_fetch_row($result)){
          ...
          <i class="icon icon-form-radio"></i>
														  ...
														  <div class="item-inner" onclick="selpacc('.$IDapp.','.$IDserv.')">
															<div class="item-title-row">
															  <div class="item-title" style="font-size:13px;">'.$row5['1'].'</div>
															  <div class="item-after" style="width:10%;font-size:13px; color:#1f63d3;">€ '.$prezzo.'</div>
															</div>
