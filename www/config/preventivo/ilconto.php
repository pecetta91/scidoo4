			...
			while($row2=mysqli_fetch_row($result2)){
				...
				if($IDoraripren!=0){
					...
					if($tipolimpacc==5){
							...
							<i class="f7-icons" style="color:#b42125;" onclick="eliminapaccprev('.$IDpersona.',0)">delete_round_fill</i>
							</div>
								</div>
			...
			while($row=mysqli_fetch_row($result)){
				...
				$IDserv=$row['0'];
					...
					<div class="row rowlist no-gutter">
						...
						<div class="col-10" >
							<i class="f7-icons" onclick="eliminaextraprev('.$IDoraripren.','."'".$IDrestr."'".')" style="color:#b42125;">delete_round_fill</i>
						</div>
					</div>
					';
