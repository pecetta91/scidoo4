...
if($IDtipo==4){
	$query2="SELECT  p.ID,p.time,p.durata,p.IDpren,SUM(p2.qta),p.esclusivo,s.servizio,p.sala FROM prenextra as p,prenextra2 as p2,servizi as s WHERE p.sottotip='$IDsottotip' AND p2.IDprenextra=p.ID AND FROM_UNIXTIME(p.time,'%Y-%m-%d')='$data' AND modi>'0' AND s.ID=p.extra  GROUP BY p.ID";
}else{
	$query2="SELECT  p.ID,p.time,p.durata,p.IDpren,1,p.esclusivo,s.servizio,p.IDpers,p.sala FROM prenextra as p,prenextra2 as p2,servizi as s WHERE p.IDtipo='2' AND p2.IDprenextra=p.ID AND FROM_UNIXTIME(p.time,'%Y-%m-%d')='$data'  AND modi>'0' AND p.extra=s.ID  GROUP BY p.ID ";
}
	...
	if($IDtipo==2){
		...
		$col='';
			...
			$IDprensosp=getprenotazioni($datagg,0,$IDstruttura,1,1);
				...
				<a href="#" onclick="opensosp()"  class="button button-round button-fill" style="width:150px; margin-top:-20px; float:right; margin-right:10px;"><i class="f7-icons" style="font-size:13px;">bars</i> &nbsp;&nbsp;<span id="sospesi">'.$num.'</span> Sospesi</a>
